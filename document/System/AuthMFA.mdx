...
The RODE MFA micro-service is to be used as an external service delivering the functionality around the user multi factor authentication of the RODE platform.

## Design

This microservice is a RESTful service that uses short lived tokens to authenticate the MFA request. Upon the successful MFA authentication the user's originating IP address will be added to the user specific IP whitelist. IP addresses that have not been accessed by the user within 1 week will be removed from the IP whitelist for the user.

![Infrastructure](https://rode.forske.org/diagrams/mfa/infrastructure.drawio.svg)
...
![Post Process](https://rode.forske.org/diagrams/mfa/post.drawio.svg)
